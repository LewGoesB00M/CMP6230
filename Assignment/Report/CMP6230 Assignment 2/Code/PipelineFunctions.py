...
### And also the LabelEncoder and StandardScaler
import pickle as pkl

### OS is imported so that file directories can be checked so Pickle
### can properly dump.
import os
...
def encode(df):
    ...
    # Serialize and save the fitted LabelEncoder for education to a file.
    serialize_to_file(le, "le_education")
    
    # Also label encode the previous loan defaults column.
    df["previous_loan_defaults_on_file"] = le.fit_transform(df["previous_loan_defaults_on_file"])
    
    # Serialize and save the fitted LabelEncoder for previous defaults to a file.
    serialize_to_file(le, "le_previousDefaults")
...
def oversample(x, y):
    smote = SMOTE()
    x = x.rename(str, axis="columns")  # SMOTE doesn't work with column names unless they are specifically str type.
...
### NEW TO UPDATED IMPLEMENTATION.
# This function will be used to serialize and save the LabelEncoder
# and StandardScaler. The modelAPI script in deployment will load 
# them for its own use.
def serialize_to_file(obj, file_name):
    # pkl.dump throws a FileNotFoundError if the given directory doesn't exist.
    # Therefore, using the OS package, the location for the file to be saved to is checked.
    # If it does exist, nothing happens. If it doesn't exist, it is created.
    os.makedirs(os.path.dirname(f"/home/lewis/CMP6230/{file_name}"), exist_ok=True)
    
    # pkl.dump is different to pkl.dumps.
    # dump saves to a file, whereas dumps saves to a variable.
    # This will save the serialised object (encoder, scaler) to a given file name 
    # specified as the "file_name" parameter of this function.
    with open(f"/home/lewis/CMP6230/{file_name}.pkl", "wb") as file:
    	pkl.dump(obj, file)
...
def train(params, from_redis = REDIS_CONN_INFO):
    def inner():
        ...
        ### NEW TO UPDATED IMPLEMENTATION.
        # Save the fitted StandardScaler to a file so that 
        # it can be used in the modelAPI script.
        serialize_to_file(sc, "standard_scaler")
        ####################################
...
def evaluate(from_redis = REDIS_CONN_INFO):
	...
	# Retrieve the serialised model from Redis.
        ...
        # Output the metrics to stdout, which is somewhat like print.
