...
The installation process of Docker is lengthy, and can therefore be found in Appendix A. % The appendix is something you'll need to think about.
...
% because you did it weeks ago in one of the labs. This is a major issue, and you'll likely need to make a new VM to compensate for it.
% Alternatively, revert the existing VM to a snapshot taken before each installation, and screenshot the installation processes.
% This way should be shorter.

% \subsection{MLFlow}
% Running MLFlow server is only necessary once. 
% When that command is run, it also runs mlflow ui by itself.
% In future use, MLFlow's UI is accessed via mlflow ui and then localhost:5000.

% Consider moving the entire 'Initial Software Setup' section into an appendix.
...
\addcontentsline{toc}{chapter}{Appendix A - Docker installation}
